...
if(empty($_GET["brand"]) && isset($_COOKIE['select_cat']) && $querycat == ''){
...
$num = 6;
$page = (int)$_GET['page'];
$count = mysql_query("SELECT COUNT(*) FROM product WHERE vis='1' $querycat", $link);
       ...
       <p id="nav-breadcrumbs"><a href="index.php"> Главная страница </a> \ <span> Все товары </span></p>
           <ul id="options-list">
             ...
             <li><a id="select-sort">'.$sort_name.'</a>
                ...
                <ul id="sorting-list">
                  <li><a href="view.php?'.$catlink.'type='.$type.'&sort=price-asc"> От дешевых к дорогим </a></li>
                  <li><a href="view.php?'.$catlink.'type='.$type.'&sort=price-desc"> От дорогих к дешевым </a></li>
                  <li><a href="view.php?'.$catlink.'type='.$type.'&sort=popular"> Популярное </a></li>
                  <li><a href="view.php?'.$catlink.'type='.$type.'&sort=news"> Новинки </a></li>
                  <li><a href="view.php?'.$catlink.'type='.$type.'&sort=brand"> По алфавиту </a></li>
                </ul>
	         </li>
         </ul>
...
<?php	//постраничная навигация
	...
	$pstr_prev = '<ul><li><a class="pstr-prev" href="view.php?page='.($page - 1).'">&lt;</a></li>';//}

	$pstr_next = '<li><a class="pstr-next" href="view.php?page='.($page + 1).'">&gt;</a></li>';

	if($page - 5 > 0) $page5left = '<li><a href="view.php?page='.($page - 5).'">'.($page - 5).'</a></li>';
	if($page - 4 > 0) $page4left = '<li><a href="view.php?page='.($page - 4).'">'.($page - 4).'</a></li>';
	if($page - 3 > 0) $page3left = '<li><a href="view.php?page='.($page - 3).'">'.($page - 3).'</a></li>';
	if($page - 2 > 0) $page2left = '<li><a href="view.php?page='.($page - 2).'">'.($page - 2).'</a></li>';
	if($page - 1 > 0) $page1left = '<li><a href="view.php?page='.($page - 1).'">'.($page - 1).'</a></li>';

	if($page + 5 <= $total) $page5right = '<li><a href="view.php?page='.($page + 5).'">'.($page + 5).'</a></li>';
	if($page + 4 <= $total) $page4right = '<li><a href="view.php?page='.($page + 4).'">'.($page + 4).'</a></li>';
	if($page + 3 <= $total) $page3right = '<li><a href="view.php?page='.($page + 3).'">'.($page + 3).'</a></li>';
	if($page + 2 <= $total) $page2right = '<li><a href="view.php?page='.($page + 2).'">'.($page + 2).'</a></li>';
	if($page + 1 <= $total) $page1right = '<li><a href="view.php?page='.($page + 1).'">'.($page + 1).'</a></li>';


	if($page + 5 < $total){
	$strtotal = '<li><p class="nav-point">...</p></li><li><a href="view.php?page='.$total.'">'.$total.'</a></li></ul>';
	} else {
	$strtotal = "";
	}

	echo '<div class="pstrnav"><ul>';
	echo $pstr_prev.$page5left.$page4left.$page3left.$page2left.$page1left."<li><a class='pstr-active' href='view.php?page=".$page."'>".$page."</a></li>".$page1right.$page2right.$page3right.$page4right.$page5right.$strtotal.$pstr_next;
	echo '</ul></div>';
